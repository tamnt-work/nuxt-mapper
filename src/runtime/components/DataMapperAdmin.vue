...
const filteredLogs = computed(() => {
  ...
  return commandStatus.value.logs
    .filter((log) => {
      ...
      if (!filterTypes.value.info && /^[ℹ️]\s/.test(log)) return false
...
// Modify updateMaxLogLines to handle existing logs properly
function updateMaxLogLines(value: number) {
  maxLogLines.value = value
  if (commandStatus.value.logs && commandStatus.value.logs.length > value) {
    // Keep the most recent logs by slicing from the end
    commandStatus.value.logs = commandStatus.value.logs.slice(-value)
  }
}
